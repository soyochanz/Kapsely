-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Fix feed RPCs to include posts, images & videos as separate events
-- 2026-04-19
-- ─────────────────────────────────────────────────────────────────────────────

-- ─── get_following_feed ───────────────────────────────────────────────────────
-- Rules:
--   • Show capsule creations AND item uploads as separate events
--   • Group item uploads by batch_id (extracted from caption) to avoid spam
--   • Only from followed users

DROP FUNCTION IF EXISTS public.get_following_feed(UUID, TEXT, INT, INT);
...
BEGIN
    ...
    following AS (
        SELECT following_id FROM follows WHERE follower_id = req_user_id
    ),
    -- Capsules from followed users
    base_capsules AS (
        SELECT c.*
        FROM capsules c
        WHERE c.owner_id IN (SELECT following_id FROM following)
    ...
    -- Activity Union: Creations and Item Uploads
    activity AS (
        -- 1. Capsule Creations (only if within cutoff)
        SELECT 
            c.id AS event_id,
            c.id AS capsule_id,
        ...
        -- 2. Item Uploads (grouped by batch_id, only if within cutoff)
        SELECT
    ...
    -- Final scoring and data hydration
    scored AS (
        SELECT
            act.event_id                                            AS id,
            bc.created_at,
            ...
            -- Latest image/video item (at the time of the event)
            (
                SELECT jsonb_build_object(
                    'id',            ci2.id,
            ...
            TRUE AS is_followed,
            -- Scoring
            0.0::NUMERIC AS affinity_score,
            0.0::NUMERIC AS momentum_score,
            0.0::NUMERIC AS event_horizon_score,
            -- Recency score: based on activity date
            GREATEST(0.0, 100.0 * EXP(-0.06 *
                EXTRACT(EPOCH FROM (NOW() - act.a_date) / 3600.0)
            ))                                                       AS recency_score,
            -- Engagement
            LEAST(50.0, COALESCE((SELECT COUNT(*) FROM likes l3 WHERE l3.capsule_id = bc.id), 0) * 3.0
                      + COALESCE((SELECT COUNT(*) FROM comments c3 WHERE c3.capsule_id = bc.id), 0) * 5.0) AS engagement_score,
            -- Seen penalty
            CASE WHEN EXISTS(SELECT 1 FROM feed_impressions fi WHERE fi.user_id = req_user_id AND fi.capsule_id = bc.id) THEN -30.0 ELSE 0.0 END AS seen_penalty,
            0.0::NUMERIC AS discovery_bonus
        FROM activity act
    ...
    SELECT
        ...
        (s.recency_score + s.engagement_score + s.seen_penalty + s.discovery_bonus) AS total_score
    FROM scored s
...
-- Rules:
--   • Show capsule creations AND item uploads as separate events
--   • No age limit for explore (discovery focused)
--   • Diversity: limit to 2 events per user per batch

DROP FUNCTION IF EXISTS public.get_explore_feed(UUID, TEXT, INT, INT);
...
BEGIN
    ...
    scored AS (
        SELECT
            ...
            EXISTS(SELECT 1 FROM likes l2 WHERE l2.capsule_id = bc.id AND l2.user_id = req_user_id) AS is_liked,
            EXISTS(SELECT 1 FROM comments c2 WHERE c2.capsule_id = bc.id AND c2.user_id = req_user_id) AS is_commented,
            EXISTS(SELECT 1 FROM follows f WHERE f.follower_id = req_user_id AND f.following_id = bc.owner_id) AS is_followed,
            -- Affinity
            ...
            -- Recency score: softer decay for explore
...
GRANT EXECUTE ON FUNCTION public.get_following_feed(UUID, TEXT, INT, INT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_explore_feed(UUID, TEXT, INT, INT) TO anon, authenticated;
