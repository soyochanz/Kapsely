-- Give opened capsules with visible media more weight in the feed.
-- They add color to Kapsely, so they should compete near the top like strong social posts.
    ...
    SELECT jsonb_agg(f) INTO v_feed FROM (
        ...
        base_capsules AS (
            SELECT c.*,
                   ...
                   CASE
                     WHEN c.status = 'opened'
                      AND (
                        c.cover_url IS NOT NULL OR EXISTS (
                            SELECT 1
                            FROM capsule_items ci_open
                            WHERE ci_open.capsule_id = c.id
                              AND ci_open.is_story = FALSE
                              AND ci_open.media_type IN ('image', 'video')
                              AND ci_open.moderation_status != 'rejected'
                        )
                      )
                     THEN 220
                     WHEN c.status = 'opened' THEN 110
                     ELSE 0
                   END AS open_capsule_score,
        ...
        SELECT
            ...
            (
                bc.freshness_score + bc.open_capsule_score + bc.opening_soon_score + bc.follow_score + bc.follow_age_score + bc.interaction_score + bc.mutual_score +
