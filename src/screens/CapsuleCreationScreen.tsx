...
const typeCardS = StyleSheet.create({
    card: {
        ...
        shadowColor: L.shadowMd, shadowOpacity: 1, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2,
        overflow: 'hidden',
    },
...
function SealAnimation({ accent, modelUri, modelOpenUri, onDone }: {
    ...
    const particles = useRef(
        Array.from({ length: 8 }, (_, i) => {
            ...
            return {
                ...
                color: [accent, accent + 'BB', '#F59E0B', '#10B981', '#3B82F6', '#EC4899', accent, '#F59E0B'][i],
            };
        })
    ).current;
    ...
    useEffect(() => {
        ...
        Animated.parallel([
            ...
            // Breathing pulse on open capsule
            ...
            // Phase 2: Items fly in after 700ms
            setTimeout(() => {
    ...
    return (
        <View style={[StyleSheet.absoluteFill, { backgroundColor: '#fff', zIndex: 3000, alignItems: 'center', justifyContent: 'center' }]}>
            {/* Ambient glow */}
            <Animated.View style={{
            ...
            {items.map((item, i) => (
                <Animated.View key={i} style={{
                    position: 'absolute',
                    bottom: '40%',
                    left: width / 2 - 28 + (i - 1) * 72,
                    transform: [{ translateY: item.y }, { scale: item.sc }],
                    opacity: item.op,
                }}>
                    <View style={{
                        width: 56, height: 56, borderRadius: 16,
                        backgroundColor: accent + '12',
                        borderWidth: 1.5, borderColor: accent + '28',
                        alignItems: 'center', justifyContent: 'center',
                        shadowColor: accent, shadowOpacity: 0.25, shadowRadius: 14,
            ...
            <Animated.View style={{
                position: 'absolute',
                opacity: stage === 'sealed' ? new Animated.Value(0) : capOpacity,
                transform: [{ scale: Animated.multiply(capScale, breathe) }],
            }}>
                <Image
                    source={{ uri: modelOpenUri || modelUri }}
            ...
            <Animated.View style={{
                ...
                <Animated.View style={{
                    ...
                    alignItems: 'center', justifyContent: 'center',
                    shadowColor: accent, shadowOpacity: 0.45, shadowRadius: 14,
            ...
            <Animated.View style={{
                ...
                alignItems: 'center', opacity: doneOpacity,
            }}>
                <Text style={{ fontSize: 23, fontFamily: Fonts.bold, color: L.text, letterSpacing: -0.4 }}>
                    Sealed for the future ✦
                </Text>
                <Text style={{ fontSize: 14, color: L.textSec, fontFamily: Fonts.regular, marginTop: 5 }}>
...
export default function CapsuleCreationScreen() {
    ...
    const toggleInviteUser = (u: any) => {
        if (invitedUsers.some(iu => iu.id === u.id)) {
            ...
            if (invitedUsers.length >= 9) {
                Alert.alert('Limit reached', 'You can invite up to 9 members.');
                return;
            }
    ...
    const accent = cfg?.accent ?? '#6B4FBF';
    ...
    useEffect(() => {
        ...
        // Hide bottom tab bar from parent navigator
    ...
    useEffect(() => {
        const check = async () => {
            ...
            try {
                ...
                if (user) {
                    const { count: lc } = await supabase.from('capsules')
                        .select('*', { count: 'exact', head: true })
                        .eq('owner_id', user.id).eq('type', 'legacycap').eq('status', 'sealed');
                    if (lc != null) setHasLegacyCap(lc > 0);

                    const { count: ic } = await supabase.from('capsules')
                        .select('*', { count: 'exact', head: true })
                        .eq('owner_id', user.id).eq('type', 'instacap').eq('status', 'sealed');
    ...
    const goNext = () => {
        ...
        if (stepIndex < STEPS.length - 1) goToStep(STEPS[stepIndex + 1], 1);
    };

    const goBack = () => {
        if (stepIndex > 0) goToStep(STEPS[stepIndex - 1], -1);
    };
    ...
    const sealCapsule = async () => {
        ...
        try {
            ...
            if (selectedType === 'legacycap') {
                const { count } = await supabase.from('capsules')
                    .select('*', { count: 'exact', head: true })
                    .eq('owner_id', user.id).eq('type', 'legacycap').eq('status', 'sealed');
                if (count && count >= 1) {
                    Alert.alert('Limit reached', 'You already have an active LegacyCap.'); setSealing(false); return;
                }
            }
            if (selectedType === 'instacap') {
                const { count } = await supabase.from('capsules')
                    ...
                    .eq('owner_id', user.id).eq('type', 'instacap').eq('status', 'sealed');
                if (count && count >= 5) {
                    Alert.alert('Limit reached', 'You have 5 active InstaCaps. Open or delete one first.'); setSealing(false); return;
                }
            }
            ...
            if (isShared && invitedUsers.length > 0 && newCapsule) {
                const inviteRows = invitedUsers.map(u => ({
                    capsule_id: newCapsule.id,
                    user_id: u.id,
                    status: 'pending'
                }));
                const { error: inviteError } = await supabase.from('capsule_invites').insert(inviteRows);
            ...
            setTimeout(() => {
                ...
                navigation.reset({
                    index: 1,
                    routes: [{ name: 'Main' }, { name: 'CapsuleDetail', params: { capsuleId: newCapsule.id } }],
                });
            }, 5800);
        } catch (e: any) {
        ...
        <View style={s.root}>
            ...
            <View style={[s.header, { paddingTop: insets.top + 4 }]}>
                ...
                <View style={s.stepChip}>
                    <Text style={[s.stepChipText, { color: accent }]}>{stepIndex + 1} of {STEPS.length}</Text>
                </View>
            </View>
                ...
                {currentStep === 'type' && (
                    <View style={s.page}>
                        ...
                        <View style={{ marginTop: 24 }}>
                            <TypeCard typeKey="legacycap" isSelected={selectedType === 'legacycap'}
                                isLocked={hasLegacyCap || loadingLimits} onPress={() => setSelectedType('legacycap')} />
                            <TypeCard typeKey="instacap" isSelected={selectedType === 'instacap'}
                                isLocked={activeInstaCapCount >= 5 || loadingLimits} onPress={() => setSelectedType('instacap')} />
                            <TypeCard typeKey="eventcap" isSelected={selectedType === 'eventcap'}
                                isLocked={!activeEvent || loadingLimits} onPress={() => setSelectedType('eventcap')} />
                        </View>
                ...
                {currentStep === 'design' && (
                    <View style={s.page}>
                        ...
                        <View style={s.designHero}>
                            <View style={[s.designGlow, { backgroundColor: accent + '22' }]} />
                            ...
                            {availableModels.filter(m => {
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.8}
                                        style={[s.modelCard, isActive && { borderColor: accent, backgroundColor: cfg?.light }]}
                                    >
                                        <Image source={{ uri: model.image }} style={s.modelImg} resizeMode="contain" />
                                        <Text style={[s.modelLabel, isActive && { color: accent }]} numberOfLines={1}>{model.label}</Text>
                                        {isActive && (
                                            <View style={[s.modelCheck, { backgroundColor: accent }]}>
                    ...
                    <View style={s.page}>
                        ...
                        <View style={[s.previewCard, { borderColor: accent + '35', backgroundColor: cfg?.light }]}>
                            ...
                            <View style={{ flex: 1, minWidth: 0 }}>
                                <Text style={[s.previewTitle, { color: title ? L.text : L.textMuted }]} numberOfLines={1}>
                            ...
                            <View style={{ marginTop: 10, gap: 12 }}>
                                <View style={[s.privacyRow, { borderColor: accent + '35', backgroundColor: cfg?.light }]}>
                                    ...
                                    <Switch 
                                        ...
                                        trackColor={{ false: L.border, true: accent + '55' }}
                                        thumbColor={isShared ? accent : L.textMuted} 
                                    />
                                </View>
                                ...
                                {isShared && (
                                    <View style={{ gap: 10 }}>
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderRadius: 16, borderWidth: 1.5, borderColor: L.border, paddingHorizontal: 16, height: 50 }}>
                                            <Ionicons name="search" size={16} color={L.textMuted} />
                                            <TextInput
                                                style={{ flex: 1, fontSize: 14, fontFamily: Fonts.semiBold, color: L.text }}
                                        ...
                                        {userSearchResults.length > 0 && (
                                            <View style={{ backgroundColor: '#fff', borderRadius: 16, borderWidth: 1.5, borderColor: L.border, maxHeight: 180, overflow: 'hidden', elevation: 2 }}>
                                                {userSearchResults.map(u => {
                                                    ...
                                                    return (
                                                        <TouchableOpacity 
                                                            key={u.id} 
                                                            disabled={isInvited}
                                                            onPress={() => { toggleInviteUser(u); setUserSearchQuery(''); setUserSearchResults([]); }} 
                                                            style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderBottomWidth: 1, borderBottomColor: L.border }}
                                                        >
                                                            <View>
                                                            ...
                                                            {isInvited ? (
                                                                ...
                                                                <Ionicons name="person-add" size={16} color={L.textSec} />
                                                            )}
                                                        </TouchableOpacity>
                                                    );
                                                })}
                        ...
                        {selectedType === 'legacycap' && (
                            <View style={[s.fixedCard, { borderColor: accent + '40', backgroundColor: cfg?.light }]}>
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={s.fixedSub}>LegacyCaps always lock for exactly 5 years. Unchangeable after sealing.</Text>
                                </View>
                            </View>
                        )}
                        ...
                        {selectedType === 'instacap' && (<>
                                ...
                                {PRESETS.map(p => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {isActive && (
                                                <View style={[s.presetCheck, { backgroundColor: accent }]}>
                                                    <Ionicons name="checkmark" size={8} color="#fff" />
                                                </View>
                                            )}
                                        </TouchableOpacity>
                            ...
                            {showCustomSlider && (
                                <View style={[s.sliderCard, { borderColor: accent + '40' }]}>
                                    ...
                                    <DurationSlider
                                        days={customDays} onChange={setCustomDays}
                                        accent={accent} daysToLabel={daysToLabel}
                        ...
                        {selectedType === 'eventcap' && (
                            <View style={[s.fixedCard, { borderColor: accent + '40', backgroundColor: cfg?.light }]}>
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={s.fixedSub}>All EventCaps open simultaneously worldwide when the event ends.</Text>
                                </View>
                            </View>
                        )}
                    </View>
                ...
                {currentStep === 'review' && (
                    <View style={s.page}>
                        ...
                        <View style={s.warningBox}>
                            <Ionicons name="information-circle-outline" size={16} color="#8B6A2A" />
                        ...
                        <TouchableOpacity
                            onPress={sealCapsule} disabled={sealing} activeOpacity={0.88}
            ...
            {/* Bottom Nav — always visible above keyboard */}
            {currentStep !== 'review' && (
                <View style={[s.bottomNav, { paddingBottom: Math.max(insets.bottom + 6, 18) }]}>
                    ...
                    <TouchableOpacity
                        onPress={goNext} disabled={!isNextEnabled} activeOpacity={0.88}
...
const s = StyleSheet.create({
    ...
    stepChip: {
        paddingHorizontal: 10, paddingVertical: 4, backgroundColor: L.surface,
    ...
    eyebrow: { fontSize: 10, fontFamily: Fonts.bold, color: L.textMuted, letterSpacing: 2, marginBottom: 8 },
    ...
    privacyRow: {
        ...
        borderRadius: 18, borderWidth: 1.5, padding: 14, marginTop: 16,
    },
    ...
    // Design step
    designHero: {
        alignItems: 'center', justifyContent: 'center', paddingVertical: 24, position: 'relative',
    },
    ...
    modelCard: {
        width: 100, padding: 10, borderRadius: 18, borderWidth: 1.5, borderColor: L.border,
        backgroundColor: L.surface, alignItems: 'center', gap: 5, position: 'relative',
        shadowColor: L.shadow, shadowOpacity: 1, shadowRadius: 4, shadowOffset: { width: 0, height: 1 }, elevation: 1,
    },
    modelImg: { width: 58, height: 74 },
    modelLabel: { fontSize: 11, fontFamily: Fonts.semiBold, color: L.textSec, textAlign: 'center' },
    modelCheck: {
        position: 'absolute', top: 6, right: 6,
        width: 16, height: 16, borderRadius: 8, alignItems: 'center', justifyContent: 'center',
    },
    chainCard: {
        width: 68, alignItems: 'center', padding: 8, gap: 5,
        borderRadius: 16, borderWidth: 1.5, borderColor: L.border, backgroundColor: L.surface,
    },
    chainIcon: {
        width: 36, height: 36, borderRadius: 10,
        backgroundColor: L.surfaceAlt, alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
    },
    ...
    previewCard: {
        flexDirection: 'row', alignItems: 'center', gap: 12,
        borderRadius: 18, borderWidth: 1.5, padding: 14, marginTop: 20, marginBottom: 22,
    },
    ...
    fieldLabel: { fontSize: 10, fontFamily: Fonts.bold, color: L.textMuted, letterSpacing: 1.5, marginBottom: 8 },
    fieldInput: {
    ...
    fieldTextArea: {
        minHeight: 130, fontSize: 15, fontFamily: Fonts.regular,
    ...
    infoChipTitle: { fontSize: 13, fontFamily: Fonts.semiBold, marginBottom: 3 },
    infoChipSub: { fontSize: 12, color: L.textSec, fontFamily: Fonts.regular, lineHeight: 17 },
    ...
    fixedCard: {
        ...
        borderRadius: 20, padding: 20, borderWidth: 1.5, marginTop: 20,
    },
    ...
    sliderCard: {
        ...
        backgroundColor: L.surface, marginBottom: 14,
        shadowColor: L.shadow, shadowOpacity: 1, shadowRadius: 4, shadowOffset: { width: 0, height: 1 }, elevation: 1,
    },
    ...
    // Review
    reviewHero: {
        alignItems: 'center', paddingVertical: 10, marginBottom: 22, marginTop: 16, position: 'relative',
    },
    ...
    warningBox: {
        flexDirection: 'row', alignItems: 'flex-start', gap: 10,
        borderRadius: 14, borderWidth: 1, borderColor: '#F0D090',
        backgroundColor: '#FEF9EC', padding: 14, marginBottom: 22,
    },
    warningText: { fontSize: 12, color: '#7A5A1A', fontFamily: Fonts.regular, flex: 1, lineHeight: 18 },

    sealBtn: {
        flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
        gap: 10, borderRadius: 20, paddingVertical: 18, marginBottom: 10,
        shadowColor: L.shadowMd, shadowOpacity: 0.5, shadowRadius: 12,
    ...
    bottomNav: {
        ...
        shadowColor: L.shadowMd, shadowOpacity: 1, shadowRadius: 14,
        shadowOffset: { width: 0, height: -4 }, elevation: 10,
    },
    backBtn: {
        width: 52, height: 52, borderRadius: 16,
        backgroundColor: L.surfaceAlt, borderWidth: 1, borderColor: L.border,
        alignItems: 'center', justifyContent: 'center',
    },
    nextBtn: {
        flex: 1, height: 52, borderRadius: 16,
        alignItems: 'center', justifyContent: 'center',
        shadowColor: L.shadowMd, shadowOpacity: 0.35, shadowRadius: 8,
